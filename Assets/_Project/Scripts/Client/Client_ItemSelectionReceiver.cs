...
public sealed class Client_ItemSelectionReceiver : MonoBehaviour
{
    ...
    private struct QueuedChoice
    {
        ...
        public ClientItemSelectionChoicePacket packet;
    }
    ...
    public event Action<ServerItemSelectionOfferPacket> OfferReceived;
    public event Action<ServerItemSelectionResultPacket> ResultReceived;
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("[Client_ItemSelectionReceiver] NetworkManager.Singleton is null.", this);
            enabled = false;
            return;
        }
    ...
    {
        ClientItemSelectionChoicePacket packet = new ClientItemSelectionChoicePacket
        {
    ...
    private void OnOfferReceived(ulong senderClientId, FastBufferReader reader)
    {
        ...
        if (!ServerItemSelectionOfferPacket.TryRead(ref reader, out ServerItemSelectionOfferPacket packet))
        {
            return;
        }
    ...
    private void OnResultReceived(ulong senderClientId, FastBufferReader reader)
    {
        ...
        if (!ServerItemSelectionResultPacket.TryRead(ref reader, out ServerItemSelectionResultPacket packet))
        {
            return;
        }
    ...
    private void FlushQueuedOffers()
    {
        ...
        for (int i = 0; i < queuedOffers.Count; i++)
        {
            ...
            ServerItemSelectionOfferPacket packet = queuedOffers[i].packet;
    ...
    private void FlushQueuedResults()
    {
        ...
        for (int i = 0; i < queuedResults.Count; i++)
        {
            ...
            ServerItemSelectionResultPacket packet = queuedResults[i].packet;
    ...
    private void FlushQueuedChoices()
    {
        ...
        for (int i = 0; i < queuedChoices.Count; i++)
        {
            ...
            ClientItemSelectionChoicePacket packet = queuedChoices[i].packet;
    ...
    private void SendChoiceNow(ClientItemSelectionChoicePacket packet)
    {
