...
public sealed class Client_ItemSelectionView : MonoBehaviour
{
    [SerializeField] private Client_ItemSelectionReceiver receiver;
    [SerializeField] private ItemEffectCatalog itemEffectCatalog;
    [SerializeField] private Client_ItemSelectionPanel selectionPanel;
    ...
    private readonly Queue<ServerItemSelectionOfferPacket> queuedOffers = new Queue<ServerItemSelectionOfferPacket>();
    ...
    private void OnEnable()
    {
        ...
        if (receiver == null)
        {
            Debug.LogWarning("[Client_ItemSelectionView] ItemSelectionReceiver is not assigned.", this);
            return;
        }
    ...
    private void OnOfferReceived(ServerItemSelectionOfferPacket packet)
    {
        if (isOpen)
        {
    ...
    private void OnResultReceived(ServerItemSelectionResultPacket packet)
    {
    ...
    private void ResolveReceiver()
    {
        if (receiver == null)
        {
            receiver = FindAnyObjectByType<Client_ItemSelectionReceiver>();
        }
    }
    ...
    private void ResolvePanel()
    {
        if (selectionPanel == null)
        {
            selectionPanel = GetComponentInChildren<Client_ItemSelectionPanel>(true);
        }
    }
    ...
    private bool HasPanel()
    {
        ...
        if (!hasLoggedMissingPanel)
        {
            ...
            Debug.LogWarning("[Client_ItemSelectionView] ItemSelectionPanel is not assigned.", this);
        }
    ...
    private void WarnMissingCatalogOrData(int itemDataId)
    {
        if (itemEffectCatalog == null)
        {
            if (!hasLoggedMissingCatalog)
            {
                ...
                Debug.LogWarning("[Client_ItemSelectionView] ItemEffectCatalog is not assigned.", this);
            }
        ...
        if (itemDataId > 0)
        {
            Debug.LogWarning($"[Client_ItemSelectionView] ItemData id {itemDataId} is not found.", this);
        }
    }
}
